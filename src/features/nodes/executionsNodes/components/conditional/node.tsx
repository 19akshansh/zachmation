...
export type ConditionalNodeData = ConditionalFormValues & {
  status?: NodeStatus;
};
...
export const ConditionalNode = (props: NodeProps<ConditionalNodeType>) => {
  ...
  const labels =
    mode === "if"
      ? ["true", "false"]
      : [...cases.map((item) => item.label), "default"];
  ...
  return (
    <>
      ...
      <WorkflowNode
        ...
        description={
          mode === "if" ? "Route on a condition" : "Route by matching a value"
        }
          ...
          <BaseNodeContent className="p-0">
            ...
            <div className="px-3 py-3">
              <div className="rounded-md bg-muted/40 px-2.5 py-2 text-xs">
                <span className="text-muted-foreground">
                  {mode === "if" ? "Condition" : "Value"}
                </span>
                <p className="mt-1 truncate font-mono font-medium">
                  {mode === "if"
                    ? props.data.leftValue || "Configure condition"
                    : props.data.switchValue || "Configure switch"}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-1 border-t px-3 py-2.5">
              {labels.map((label) => (
                <div
                  key={label}
                  className="relative flex h-7 items-center justify-end pr-5 text-xs font-medium text-muted-foreground"
                >
                  <span
                    className={
                      label === "default"
